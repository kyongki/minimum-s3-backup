...
고객들은 Amazon S3의 99.999999999%(11 nines) 내구성과 검증된 운영 안정성을 신뢰하며 가장 소중한 데이터를 맡깁니다. 당연히 랜섬웨어, 실수로 인한 삭제, 운영자 오류로부터 데이터를 보호하고 싶어 하지만, 막대한 데이터 규모와 백업 비용 앞에서 많은 고객이 행동을 망설입니다.

행동하지 않았을 때의 결과는 심각합니다. 수백 테라바이트를 관리하는 일부 조직은 오브젝트별 백업 과금이 현실적이지 않아 도입을 포기하고, 핵심 데이터가 체계적인 복구 전략 없이 노출된 상태로 운영됩니다. 또 다른 조직은 운영자 실수나 악의적 행위로 되돌릴 수 없는 데이터 손실이 발생한 후에야 — 복원할 백업이 아무것도 없다는 사실을 깨닫게 됩니다.

AWS Backup for S3는 중앙 집중식 관리, 연속 백업, 원활한 시점 복구를 제공하는 가장 포괄적인 솔루션입니다. 하지만 대규모 환경 — 예를 들어 **500TB, 5억 개 오브젝트** — 에서는 비용이 상당할 수 있으며, 오브젝트별 과금만으로도 큰 금액에 달합니다.

비용 때문에 백업을 포기한 고객을 위해, 이 아키텍처는 현실적인 대안을 제시합니다. **Versioning**, **Cross-Account Replication**, **Object Lock**, **Lifecycle to Glacier Deep Archive**, **S3 Metadata**, **S3 Batch Operations** 등 S3 네이티브 기능을 조합하여 AWS Backup 대비 **60~80% 낮은 비용**으로 데이터를 보호할 수 있으며, 가장 대규모이고 비용에 민감한 워크로드에서도 백업을 실현할 수 있습니다. AWS Backup보다 운영 부담은 더 크지만, 500TB 환경에서 약 **월 $600**으로 필수적인 데이터 보호를 제공합니다.
...
> **참고:** Glacier IR로 전환되는 것은 noncurrent 버전만 해당됩니다. noncurrent 버전은 오브젝트가 덮어쓰기(overwrite)되거나 삭제될 때만 생성됩니다. 현재(최신) 버전은 원래 스토리지 클래스에 그대로 유지됩니다. 실제 Glacier IR 저장 용량은 전체 버킷 크기가 아닌 데이터 변경률에 따라 달라집니다.
...
```bash
# 복원 완료 카운트 (최근 1시간 TriggeredRules 메트릭 조회)
# macOS: -d '1 hour ago' 대신 -v-1H 사용
...
> **참고:** EventBridge rule은 restore가 완료되기 **전에** 생성해야 합니다. 이미 복원이 완료된 후에 rule을 생성하면, 과거 이벤트는 소급 캡처되지 않으므로 `TriggeredRules` 메트릭이 `None`을 반환합니다. 이 경우 아래 옵션 B로 복원 상태를 확인하세요.
...
```bash
# manifest ETag 재조회 (Step 1 이후 셸 세션이 변경되었을 수 있음)
MANIFEST_ETAG=$(aws s3api head-object \
  --bucket ${OPS_BUCKET_B} \
  --key pitr-manifests/dr-2024-07-15/manifest.csv \
  --query ETag --output text \
...
```bash
# 복사 작업 상태 확인 (<COPY_JOB_ID>를 위에서 반환된 job ID로 교체)
aws s3control describe-job \
  --account-id ${ACCOUNT_B} \
  --job-id <COPY_JOB_ID> \
  --region ${REGION} \
...
# (선택) 복원 버킷의 오브젝트 확인
# 대규모 버킷(수억 개 오브젝트)에서는 LIST 요청 비용이 크고 시간이 오래 걸릴 수 있습니다.
# 이 경우 위의 describe-job 결과로 확인하는 것을 권장합니다.
# aws s3 ls s3://${RESTORE_BUCKET_B}/ --recursive --summarize --profile account-b
```

`Status`가 `Complete`이고 `Succeeded`가 `Total`과 일치하면 DR 복구가 완료된 것입니다.
